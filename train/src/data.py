...
def load_data(opt, tokenizer):
    ...
    train_data_paths = opt.train_data if isinstance(opt.train_data, list) else [opt.train_data]

    for path in train_data_paths:
        logger.info(f"Loading dataset from: {path} with mode: {opt.loading_mode}")
        data = load_dataset(path, opt.loading_mode)

        if data is None:
            logger.warning(f"WARNING: No data returned for {path}")
            continue

        dset = Dataset(data, opt.chunk_length, tokenizer, opt)
        logger.info(f"{path}: loaded {len(dset)} samples")
        datasets[path] = dset

    if not datasets:
        logger.error("No datasets were loaded. Returning empty MultiDataset.")
